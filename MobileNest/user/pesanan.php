...
<head>
    ...
    <style>
        ...
        .card-pesanan { border-left: 4px solid #007bff; transition: box-shadow 0.3s; }
        .card-pesanan:hover { box-shadow: 0 0.5rem 1rem rgba(0,0,0,0.15); }
    </style>
</head>
<body>
        ...
        <div class="row mb-4">
            <div class="col-md-6">
                <div class="btn-group" role="group">
                    <a href="pesanan.php?status=semua" class="btn btn-outline-primary <?php echo $filter_status === 'semua' ? 'active' : ''; ?>">Semua</a>
                    <a href="pesanan.php?status=Pending" class="btn btn-outline-warning <?php echo $filter_status === 'Pending' ? 'active' : ''; ?>">⏳ Pending</a>
                    <a href="pesanan.php?status=Diproses" class="btn btn-outline-info <?php echo $filter_status === 'Diproses' ? 'active' : ''; ?>">⚙️ Diproses</a>
                    <a href="pesanan.php?status=Dikirim" class="btn btn-outline-primary <?php echo $filter_status === 'Dikirim' ? 'active' : ''; ?>">🚚 Dikirim</a>
                    <a href="pesanan.php?status=Selesai" class="btn btn-outline-success <?php echo $filter_status === 'Selesai' ? 'active' : ''; ?>">✅ Selesai</a>
                </div>
            </div>
        </div>
        <div class="row">
                            ...
                            <div class="card-body">
                                <div class="row align-items-center">
                                    <div class="col-md-6">
                                        ...
                                        <p class="text-muted mb-1">📅 <?php echo date('d M Y H:i', strtotime($item['tanggal_transaksi'])); ?></p>
                                        <p class="text-muted mb-0">📦 <?php echo $item['jumlah_item']; ?> item | 💳 <?php echo ucfirst($item['metode_pembayaran']); ?></p>
                                    </div>
                                    <div class="col-md-3">
                                    ...
                                    <div class="col-md-3 text-end">
                                        ...
                                        <div class="btn-group btn-group-sm" role="group">
                                            <button type="button" class="btn btn-info" data-bs-toggle="modal" data-bs-target="#detailModal" onclick="showDetail(<?php echo htmlspecialchars(json_encode($item)); ?>)">👁️ Detail</button>
            ...
            <?php else: ?>
                <div class="col-12">
                    <div class="alert alert-info text-center">
                        😐 Anda belum memiliki pesanan. <a href="../MobileNest/cari-produk.php">Mulai belanja sekarang!</a>
                    </div>
                </div>
            <?php endif; ?>
        </div>
    ...
    <script>
        function showDetail(pesanan) {
            ...
            const html = `<div class="row mb-3"><div class="col-6"><strong>ID Pesanan:</strong><br> #${pesanan.id_transaksi}</div><div class="col-6"><strong>Tanggal:</strong><br> ${new Date(pesanan.tanggal_transaksi).toLocaleDateString('id-ID')}</div></div><div class="row mb-3"><div class="col-6"><strong>Status:</strong><br> <span class="status-badge ${statusClass}">${pesanan.status_pesanan}</span></div><div class="col-6"><strong>Metode Pembayaran:</strong><br> ${pesanan.metode_pembayaran}</div></div>${pesanan.no_resi ? `<div class="row mb-3"><div class="col-12"><strong>No. Resi:</strong><br> ${pesanan.no_resi}</div></div>` : ''}<div class="row mb-3"><div class="col-12"><strong>Produk:</strong><br> ${pesanan.produk_list || 'N/A'}</div></div><div class="row"><div class="col-12"><strong>Total:</strong><br> <h5>Rp ${pesanan.total_harga.toLocaleString('id-ID')}</h5></div></div>`;
